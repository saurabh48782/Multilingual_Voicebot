...
exercised with a stubbed post_with_retry; the model itself is never loaded.
"""
...
def test_remote_client_sends_multipart_and_parses_response(monkeypatch: Any) -> None:
    ...
    monkeypatch.setattr(remote_module, "post_with_retry", fake_post)
    # decode_strategy is resolved app-side from cfg
    monkeypatch.setitem(cfg["stt"], "decode_strategy", "ctc")
...
def test_default_stt_routes_remote_when_url_set(monkeypatch: Any) -> None:
    monkeypatch.setitem(cfg["stt"], "remote_url", "http://stt:8002")
